class MenuContexto {
	...
	constructor(player) {
		...
		if(player.options){
			if(player.options.autoplay){
				...
				let _self = this;
				let video = document.getElementById(QiSatPlayer.ID_VIDEO);
	...
	endVideo (_self) {
		let videoTag = document.getElementById(QiSatPlayer.ID_VIDEO_TAG);
		let video = document.getElementById(QiSatPlayer.ID_VIDEO);

		let graph = document.getElementById(MenuContexto.ID_GRAPH), canvas, span;
		if(graph){
		...
		let autoplay = <HTMLInputElement>document.getElementById(QiSatPlayer.ID_AUTOPLAY);
