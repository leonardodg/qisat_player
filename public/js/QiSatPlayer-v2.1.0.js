var  ID = "#",
	...
	CLASS_BT  = "bt_ctrl",
	CLASS_TOP = "top",
	CLASS_PREVIOUS = "fi-previous",
	CLASS_NEXT = "fi-next",
	CLASS_LOGO  = "logo",
	CLASS_REFRESH = "fi-refresh",
	CLASS_PLAY  = "fi-play",
	CLASS_PAUSE = "fi-pause",
	CLASS_PREVIOUS_DISABLE = "fi-previous-disable",
	CLASS_NEXT_DISABLE = "fi-next-disable",
	CLASS_VOLUME = "fi-volume",
	CLASS_VOLUME_NONE   = "fi-volume-none",
	CLASS_WIDGET   = "fi-widget",
			...
			_self.$canvas.addEventListener("click", function() {
				if (!_self.$video.classList.contains(CLASS_HIDE)) {
					...
					if (_self.$btMain.classList.contains(CLASS_PLAY)) {
						...
						_self.$btMain.classList.remove(CLASS_PAUSE);
						_self.$btMain.classList.add(CLASS_PLAY);
					} else if (_self.$btMain.classList.contains(CLASS_REFRESH)) {
...
QiSatPlayer.prototype.createComponents = function() {
	...
	if(_self.$videoConteiner){
		...
		_self.touchstart = function(){
			clearInterval(this.intervalTouchEnd);
			if(_self.$video.classList.contains(CLASS_HIDE) &&
			(_self.$videoTop.classList.contains(CLASS_VISIBILY) || _self.$videoControls.classList.contains(CLASS_VISIBILY))){
				_self.$videoTop.classList.remove(CLASS_VISIBILY);
				_self.$videoControls.classList.remove(CLASS_VISIBILY);

				_self.$subMenu.classList.add(CLASS_HIDE);
				clearInterval(_self.$btWidget.interval);
			} else {
				_self.$videoTop.classList.add(CLASS_VISIBILY);
				_self.$videoControls.classList.add(CLASS_VISIBILY);
			}
		};
		_self.$canvas.addEventListener("touchstart", _self.touchstart);
		_self.touchend = function(){
			clearInterval(this.intervalTouchEnd);
			this.intervalTouchEnd = setTimeout(function(){ 
				if(!_self.$video.paused){
					_self.$videoTop.classList.remove(CLASS_VISIBILY);
					_self.$videoControls.classList.remove(CLASS_VISIBILY);
		...
		_self.$canvas.addEventListener("touchend", _self.touchend);
...
QiSatPlayer.prototype.controlsBar = function() {
	...
	_self.$btPrevious   = document.querySelector(CLASS+CLASS_PREVIOUS_DISABLE);
	...
	_self.$btPrevious.addEventListener("click", function() {
		var dataSubLine;
		if(!this.classList.contains(CLASS_PREVIOUS_DISABLE)){
	...
	if(_self.$barVolume){
		...
		_self.$btVolume.addEventListener("click", function() {
			if(this.classList.contains(CLASS_VOLUME)){
				if (_self.$video.muted == false) {
					...
					this.classList.add(CLASS_VOLUME_NONE);
		...
		_self.$barVolume.change = function(e) {
			...
			if(value == 0){
				...
				_self.$btVolume.classList.add(CLASS_VOLUME_NONE);
			}else if((value > 0) && (_self.$video.muted == true)){
				_self.$video.muted = false;
				_self.$btVolume.classList.remove(CLASS_VOLUME_NONE);
	...
	_self.$btMain.addEventListener("click", function() {
		...
		}else if(this.classList.contains(CLASS_REFRESH)){
			_self.$video.currentTime = 0;
			_self.$video.start();
		}
	});
	...
	if(_self.$video){
		...
		_self.$video.end = function (){
			...
			var checkNext, checkPrevious, unid, subUnid, length, videoData, $btSubVideos,
				videoData = _self.getDataVideo(),
				subLineLength;
			...
			_self.$btMain.classList.add(CLASS_REFRESH);
			...
			var btActionEvent = function(subLine){
				...
				if(dataSubLine){
					if(typeof _self.$btNext.dataset !== "undefined") {
						...
						_self.$btPrevious.dataset.subline = subLine;
					} else {
						_self.$btNext.setAttribute('data-subline', subLine);
						_self.$btPrevious.setAttribute('data-subline', subLine);
					}
		...
		_self.$video.start = function (load){
			var checkPrevious = true, btActionLength;
			...
			if( _self.$btPrevious){
				...
				if(checkPrevious){
					if(_self.$btPrevious.classList.contains(CLASS_PREVIOUS_DISABLE))
						_self.$btPrevious.classList.remove(CLASS_PREVIOUS_DISABLE);
					if(!_self.$btPrevious.classList.contains(CLASS_PREVIOUS))
						_self.$btPrevious.classList.add(CLASS_PREVIOUS);
				}else if(_self.$btPrevious.classList.contains(CLASS_PREVIOUS)){
					_self.$btPrevious.classList.remove(CLASS_PREVIOUS);
					_self.$btPrevious.classList.add(CLASS_PREVIOUS_DISABLE);
				}
			}
		...
		_self.$video.timeup = function () {
			...
			var curmins = Math.floor(_self.$video.currentTime / 60),
				...
				value = ( _self.$video.currentTime  / _self.$video.duration );
		...
		_self.$video.loadedData = function () {
			var durmins = Math.floor(_self.$video.duration / 60),
				dursecs = Math.floor(_self.$video.duration - durmins * 60);
		...
		_self.$progress.change = function (e) {
			...
			if(_self.$progress.$bar){
				value = e.offsetX / _self.$progress.offsetWidth,
					time = _self.$video.duration * value;
	...
	if(_self.options.autoplay && !_self.autoplay){
        ...
        opcoes['Reprodução automática'] = checkbox;
	}

	if(_self.options.playbackRate && !_self.$playbackRate){
		var playbackRateClick = function(){
            var valores = [0.5, 0.75, 1, 1.25, 1.5, 2];
            var playback = document.getElementById('playback');
			var index = valores.indexOf(_self.$video.playbackRate) + (this.classList.contains('fi-plus') ? 1 : -1);
		...
		playback.id = 'group-playback';

		var diminuir = document.createElement('i');
		diminuir.classList.add('fi-minus');
        diminuir.addEventListener("click", playbackRateClick, false);
		playback.appendChild(diminuir);
        
		var label = document.createElement('label');
		label.id = 'playback';
		label.textContent = '1x';
        playback.appendChild(label);

		aumentar = document.createElement('i');
		aumentar.classList.add('fi-plus');
        aumentar.addEventListener("click", playbackRateClick, false);
        playback.appendChild(aumentar);

        opcoes['Velocidade de reprodução'] = playback;
	}

    _self.$subMenu = document.createElement('div');
	_self.$subMenu.id = 'subMenu';
	_self.$subMenu.classList.add(CLASS_HIDE);
    ...
    for (var key in opcoes) {
        ...
        linha.textContent = key;
        coluna.appendChild(linha);

        linha = document.createElement("td");
        linha.style.width = '86px';
        linha.style.paddingLeft = '15px';
        linha.appendChild(opcoes[key]);
        coluna.appendChild(linha);
        corpo.appendChild(coluna);
    }

    tabela.appendChild(corpo);
    _self.$subMenu.appendChild(tabela);
    _self.$videoControls.appendChild(_self.$subMenu);

    _self.$btWidget = document.querySelector(CLASS+CLASS_WIDGET);
    _self.$btWidget.addEventListener("click", function(){
        if(_self.$subMenu.classList.contains(CLASS_HIDE)){
			_self.$subMenu.classList.remove(CLASS_HIDE);

            var degrees = 1;
            this.interval = setInterval(function(){ 
                if(navigator.userAgent.match("Chrome"))
                    _self.$btWidget.style.WebkitTransform = "rotate("+degrees+"deg)";
                else if(navigator.userAgent.match("Firefox"))
                    _self.$btWidget.style.MozTransform = "rotate("+degrees+"deg)";
                else if(navigator.userAgent.match("MSIE"))
                    _self.$btWidget.style.msTransform = "rotate("+degrees+"deg)";
                else if(navigator.userAgent.match("Opera"))
                    _self.$btWidget.style.OTransform = "rotate("+degrees+"deg)";
                else
                    _self.$btWidget.style.transform = "rotate("+degrees+"deg)";
...
QiSatPlayer.prototype.printMenuXML = function (){
	...
	var _self = this, subItem, slidesXML, unid, ulMenu, icon_more, x = 0, i, iLength =_self.itens.length, sLength, titulo,
		...
		iplus, ipage = '<i class="fi-page"></i>', ipagesmall = '<i class="fi-page small"></i>';
	...
	if(_self.$menuList){
		for (; x < iLength; x++){
			...
			if(sLength>0){
				icon_more = 'li_more';
				iplus = '<i class="fi-plus"></i>';
			}else{
				icon_more = 'li_span';
			}

			listHTML = listHTML + '<div class="li_action li_view '+icon_more+'" data-unid="'+unid+'">'+iplus+'</div>';
			listHTML = listHTML + '<li class="li_action" data-unid="'+unid+'" > <div class="li_icon">'+ipage+'</div><span>';
			...
			if(sLength>0){
				...
				for (; i < sLength; i++){
					...
					listHTML = listHTML + '<li class="li_action" data-unid="'+unid+'" ><div class="li_icon"><div></div>'+ipagesmall+'</div><span>';
					listHTML = listHTML + (titulo);
		...
		$liAction[x].addEventListener("click", function(event){
			...
			if(this.tagName == 'DIV'){
				...
				if(this.classList.contains('li_more')){
					...
					this.childNodes[0].classList.remove('fi-plus');
					this.childNodes[0].classList.add('fi-minus');

				}else if(this.classList.contains('li_less')){
					this.classList.remove('li_less');
					this.classList.add('li_more');
					this.childNodes[0].classList.remove('fi-minus');
					this.childNodes[0].classList.add('fi-plus');
				...
				if(typeof _self.$btPrevious.dataset !== "undefined") {
					datasetB = _self.$btPrevious.dataset.subline;
				} else {
					datasetB = _self.$btPrevious.getAttribute('data-subline');
				}
				...
				if(datasetB){
					_self.$btPrevious.removeAttribute('data-subline');
				}
...
QiSatPlayer.prototype.previousVideo = function(){
...
QiSatPlayer.prototype.geraLog = function(unid){
	...
	lv += 'url='+window.location.href+idN+'idN='+_self._aula+'&unidade='+_self._unid+'&nivel='+nivel+'&aula=Aula%20'+_self._aula;
